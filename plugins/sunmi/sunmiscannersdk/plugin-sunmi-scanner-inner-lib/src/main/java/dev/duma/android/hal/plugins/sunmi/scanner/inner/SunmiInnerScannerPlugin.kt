...
class SunmiInnerScannerPlugin(
    ...
    override suspend fun execute(method: String, params: String): CommandResult = mutex.withLock {
        ...
        return@withLock try {
            when (method) {
                // --- Core scan control ---
                "sunmi.scanner.inner.trigger" -> { scanner.scan(); CommandResult.Success("""{"status":"scanning"}""") }
                "sunmi.scanner.inner.stop" -> { scanner.stop(); CommandResult.Success() }
                ...
                "sunmi.scanner.inner.clearConfig" -> { scanner.clearConfig(); CommandResult.Success() }
                "sunmi.scanner.inner.isServiceConnected" -> CommandResult.Success(JSONObject().put("connected", ScannerServiceManager.isConnected()).toString())
                ...
                "sunmi.scanner.inner.getOutputEncodingCode" -> queryServiceSetting { ss ->
                    CommandResult.Success(JSONObject().put("encoding", ss.mOutCodeCharSet).toString())
                }
                ...
                "sunmi.scanner.inner.getScanResultCodeID" -> queryServiceSetting { ss ->
                    CommandResult.Success(JSONObject().put("type", ss.mOutCodeID).toString())
                }
                ...
                "sunmi.scanner.inner.isOutputBroadcastEnabled" -> queryServiceSetting { ss ->
                    CommandResult.Success(JSONObject().put("enabled", ss.mOutBroadcast == 1).toString())
                }
                ...
                "sunmi.scanner.inner.isBeep" -> queryServiceSetting { ss ->
                    CommandResult.Success(JSONObject().put("enabled", ss.mTips?.getOrNull(0)?.let { it == 1 } ?: true).toString())
                }
                ...
                "sunmi.scanner.inner.isVibrate" -> queryServiceSetting { ss ->
                    CommandResult.Success(JSONObject().put("enabled", ss.mTips?.getOrNull(1)?.let { it == 1 } ?: true).toString())
                }
                ...
                "sunmi.scanner.inner.beep" -> { beeper?.beep(); CommandResult.Success() }
                "sunmi.scanner.inner.vibrate" -> { beeper?.vibrate(); CommandResult.Success() }

                // --- Flash / Scene / Center / Virtual button ---
                "sunmi.scanner.inner.isFlash" -> queryResultSetting(SunmiHelper.SET_FLASH_CONTROL) { value ->
                    CommandResult.Success(JSONObject().put("enabled", value == "1" || value == "true").toString())
                }
                ...
                "sunmi.scanner.inner.getCenterFlagScan" -> queryServiceSetting { ss ->
                    CommandResult.Success(JSONObject().put("mode", ss.mCenterFlagScan).toString())
                }
                ...
                "sunmi.scanner.inner.getScene" -> queryResultSetting(SunmiHelper.SET_SCAN_SPECIFIC_SCENE) { value ->
                    CommandResult.Success(JSONObject().put("scene", try { value.toInt() } catch (_: Exception) { 0 }).toString())
                }
                ...
                "sunmi.scanner.inner.isVirtualFloatingScanButton" -> queryServiceSetting { ss ->
                    CommandResult.Success(JSONObject().put("enabled", ss.mTrigger?.getOrNull(0)?.let { it == 1 } ?: false).toString())
                }
                ...
                "sunmi.scanner.inner.getPrefixCharactersRemoved" -> queryServiceSetting { ss ->
                    CommandResult.Success(JSONObject().put("length", ss.mPrefixCount).toString())
                }
                ...
                "sunmi.scanner.inner.getSuffixCharactersRemoved" -> queryServiceSetting { ss ->
                    CommandResult.Success(JSONObject().put("length", ss.mSuffixCount).toString())
                }
                ...
                "sunmi.scanner.inner.isRemoveGroupSeparator" -> queryServiceSetting { ss ->
                    CommandResult.Success(JSONObject().put("enabled", ss.mRemoveGroupChar == 1).toString())
                }
                ...
                "sunmi.scanner.inner.isAdvancedFormatEnabled" -> queryServiceSetting { ss ->
                    CommandResult.Success(JSONObject().put("enabled", ss.mAdvancedFormat == 1).toString())
                }
    ...
    private suspend fun querySetting(scanner: InnerScanner, query: String): CommandResult {
        ...
        scanner.sendQuery(query, object : QueryCallback() {
            ...
            override fun onFiled(errorCode: Int) {
                deferred.complete(CommandResult.internalError("Query failed with code $errorCode"))
            }
        })
        return withTimeout(QUERY_TIMEOUT_MS) { deferred.await() }
    }

    private suspend fun queryServiceSetting(transform: (ServiceSetting) -> CommandResult): CommandResult {
        val deferred = CompletableDeferred<CommandResult>()
        InnerScanner.getInstance().sendQuery(SunmiHelper.QUERY_ALL_SETTING_INFO, object : QueryCallback() {
            override fun onSuccess(entity: Entity<*>?) {
                ...
                else deferred.complete(CommandResult.internalError("Expected ServiceSetting"))
            }
            override fun onFiled(errorCode: Int) {
                deferred.complete(CommandResult.internalError("Query failed with code $errorCode"))
            }
    ...
    private suspend fun queryResultSetting(queryString: String, transform: (String) -> CommandResult): CommandResult {
        val deferred = CompletableDeferred<CommandResult>()
        InnerScanner.getInstance().sendQuery(queryString, object : QueryCallback() {
            override fun onSuccess(entity: Entity<*>?) {
                ...
                } else deferred.complete(CommandResult.internalError("Expected Result"))
            }
            override fun onFiled(errorCode: Int) {
                deferred.complete(CommandResult.internalError("Query failed with code $errorCode"))
            }
    ...
    private suspend fun queryAdvancedFormats(): CommandResult {
        val deferred = CompletableDeferred<CommandResult>()
        InnerScanner.getInstance().sendQuery(SunmiHelper.QUERY_ADVANCED_FORMAT, object : QueryCallback() {
            override fun onSuccess(entity: Entity<*>?) {
                ...
                deferred.complete(CommandResult.Success(JSONObject().put("formats", formats).toString()))
            }
            override fun onFiled(errorCode: Int) {
                deferred.complete(CommandResult.internalError("Query failed with code $errorCode"))
            }
    ...
    private suspend fun queryBarcodesList(): CommandResult {
        val deferred = CompletableDeferred<CommandResult>()
        InnerScanner.getInstance().sendQuery(SunmiHelper.QUERY_ALL_ENABLE_CODE, object : QueryCallback() {
            override fun onSuccess(entity: Entity<*>?) {
                ...
                if (bean is CodeEnable) {
                    ...
                    deferred.complete(CommandResult.Success(JSONObject().put("barcodes", barcodes).toString()))
                } else deferred.complete(CommandResult.internalError("Expected CodeEnable"))
            }
            override fun onFiled(errorCode: Int) {
                deferred.complete(CommandResult.internalError("Query failed with code $errorCode"))
            }
    ...
    private suspend fun queryBarcode(name: String): CommandResult {
        val deferred = CompletableDeferred<CommandResult>()
        InnerScanner.getInstance().sendQuery(SunmiHelper.QUERY_ALL_ENABLE_CODE, object : QueryCallback() {
            override fun onSuccess(entity: Entity<*>?) {
                val bean = entity?.bean
                if (bean is CodeEnable) {
                    ...
                    if (idx >= 0) {
                        deferred.complete(CommandResult.Success(JSONObject().put("name", name).put("enabled", bean.enable?.getOrNull(idx) ?: false).toString()))
                    } else deferred.complete(CommandResult.badRequest("Barcode type not found: $name"))
                } else deferred.complete(CommandResult.internalError("Expected CodeEnable"))
            }
            override fun onFiled(errorCode: Int) {
                deferred.complete(CommandResult.internalError("Query failed with code $errorCode"))
    ...
    private suspend fun queryBarcodeConfig(name: String): CommandResult {
        val queryCmd = SunmiHelper.queryCodeSetting(name)
        if (queryCmd.isNullOrEmpty()) return CommandResult.badRequest("Unknown barcode type: $name")
        ...
        InnerScanner.getInstance().sendQuery(queryCmd, object : QueryCallback() {
            override fun onSuccess(entity: Entity<*>?) {
                ...
                if (bean is CodeSetting) {
                    deferred.complete(CommandResult.Success(JSONObject().put("name", name)
                        ...
                        .toString()))
                } else deferred.complete(CommandResult.internalError("Expected CodeSetting"))
            }
            override fun onFiled(errorCode: Int) {
                deferred.complete(CommandResult.internalError("Query failed with code $errorCode"))
            }
    ...
    private fun setBarcodeConfig(scanner: InnerScanner, json: JSONObject): CommandResult {
    ...
    private fun serializeEntity(entity: Entity<*>?): CommandResult {
        if (entity == null) return CommandResult.internalError("No data returned")
        val bean = entity.bean ?: return CommandResult.internalError("No data in entity")
    ...
    private fun buildMethodList() = listOf(
        ...
        MethodDescriptor("sunmi.scanner.inner.stop", "Stop active barcode scan.", "sunmi.scanner.inner", exampleParameters = """{}""", exampleOutput = """{}"""),
        MethodDescriptor("sunmi.scanner.inner.sendCommand", "Send a raw configuration command to the scanner service.", "sunmi.scanner.inner", exampleParameters = """{"command":"{\"EXTRA_SCAN_POWER\":1}"}""", exampleOutput = """{}"""),
        MethodDescriptor("sunmi.scanner.inner.sendQuery", "Query scanner configuration. Returns typed response.", "sunmi.scanner.inner", exampleParameters = """{"query":"sunmi001000"}""", exampleOutput = """{"type":"ServiceSetting","data":{}}"""),
        ...
        MethodDescriptor("sunmi.scanner.inner.setOutputEncodingCode", "Set output character encoding.", "sunmi.scanner.inner", exampleParameters = """{"encoding":0}""", exampleOutput = """{}"""),
        MethodDescriptor("sunmi.scanner.inner.getScanResultCodeID", "Get scan result code ID variant.", "sunmi.scanner.inner", exampleParameters = """{}""", exampleOutput = """{"type":0}"""),
        MethodDescriptor("sunmi.scanner.inner.setScanResultCodeID", "Set scan result code ID variant.", "sunmi.scanner.inner", exampleParameters = """{"type":1}""", exampleOutput = """{}"""),
        MethodDescriptor("sunmi.scanner.inner.isOutputBroadcastEnabled", "Check if output broadcast is enabled.", "sunmi.scanner.inner", exampleParameters = """{}""", exampleOutput = """{"enabled":true}"""),
        MethodDescriptor("sunmi.scanner.inner.setOutputBroadcastEnabled", "Enable or disable output broadcast.", "sunmi.scanner.inner", exampleParameters = """{"enabled":true}""", exampleOutput = """{}"""),
        MethodDescriptor("sunmi.scanner.inner.getBroadcastConfiguration", "Get broadcast action and key configuration.", "sunmi.scanner.inner", exampleParameters = """{}""", exampleOutput = """{"action":"com.sunmi.scanner.ACTION_DATA_CODE_RECEIVED","dataKey":"data","byteKey":"source_byte"}"""),
        MethodDescriptor("sunmi.scanner.inner.setBroadcastConfiguration", "Set broadcast action and key configuration.", "sunmi.scanner.inner", exampleParameters = """{"action":"com.sunmi.scanner.ACTION_DATA_CODE_RECEIVED","dataKey":"data"}""", exampleOutput = """{}"""),
        MethodDescriptor("sunmi.scanner.inner.getTriggerMethod", "Get trigger mode, timeout, and sleep interval.", "sunmi.scanner.inner", exampleParameters = """{}""", exampleOutput = """{"mode":0,"timeout":5000,"sleep":500}"""),
        MethodDescriptor("sunmi.scanner.inner.setTriggerMethod", "Set trigger mode, timeout, and sleep interval.", "sunmi.scanner.inner", exampleParameters = """{"mode":0,"timeout":5000}""", exampleOutput = """{}"""),
        MethodDescriptor("sunmi.scanner.inner.setTrigger", "Enable or disable the physical trigger button.", "sunmi.scanner.inner", exampleParameters = """{"enabled":true}""", exampleOutput = """{}"""),
        MethodDescriptor("sunmi.scanner.inner.isBeep", "Check if beep on scan is enabled.", "sunmi.scanner.inner", exampleParameters = """{}""", exampleOutput = """{"enabled":true}"""),
        MethodDescriptor("sunmi.scanner.inner.setBeep", "Enable or disable beep on scan.", "sunmi.scanner.inner", exampleParameters = """{"enabled":true}""", exampleOutput = """{}"""),
        MethodDescriptor("sunmi.scanner.inner.isVibrate", "Check if vibration on scan is enabled.", "sunmi.scanner.inner", exampleParameters = """{}""", exampleOutput = """{"enabled":true}"""),
        MethodDescriptor("sunmi.scanner.inner.setVibrate", "Enable or disable vibration on scan.", "sunmi.scanner.inner", exampleParameters = """{"enabled":true}""", exampleOutput = """{}"""),
        MethodDescriptor("sunmi.scanner.inner.beep", "Play a beep sound.", "sunmi.scanner.inner", exampleParameters = """{}""", exampleOutput = """{}"""),
        MethodDescriptor("sunmi.scanner.inner.vibrate", "Trigger a short vibration.", "sunmi.scanner.inner", exampleParameters = """{}""", exampleOutput = """{}"""),
        MethodDescriptor("sunmi.scanner.inner.isFlash", "Check if illumination flash is enabled.", "sunmi.scanner.inner", exampleParameters = """{}""", exampleOutput = """{"enabled":true}"""),
        MethodDescriptor("sunmi.scanner.inner.setFlash", "Enable or disable illumination flash.", "sunmi.scanner.inner", exampleParameters = """{"enabled":true}""", exampleOutput = """{}"""),
        MethodDescriptor("sunmi.scanner.inner.getCenterFlagScan", "Get center decoding mode (0=disabled, 1=centerOnly, 2=centerFirst).", "sunmi.scanner.inner", exampleParameters = """{}""", exampleOutput = """{"mode":0}"""),
        MethodDescriptor("sunmi.scanner.inner.setCenterFlagScan", "Set center decoding mode.", "sunmi.scanner.inner", exampleParameters = """{"mode":1}""", exampleOutput = """{}"""),
        MethodDescriptor("sunmi.scanner.inner.getScene", "Get active specific scene optimization.", "sunmi.scanner.inner", exampleParameters = """{}""", exampleOutput = """{"scene":0}"""),
        MethodDescriptor("sunmi.scanner.inner.setScene", "Set specific scene optimization.", "sunmi.scanner.inner", exampleParameters = """{"scene":1}""", exampleOutput = """{}"""),
        MethodDescriptor("sunmi.scanner.inner.switchSpecialScene", "Switch scanner to a special scene mode via SDK.", "sunmi.scanner.inner", exampleParameters = """{"scene":1}""", exampleOutput = """{}"""),
        MethodDescriptor("sunmi.scanner.inner.isVirtualFloatingScanButton", "Check if the virtual floating scan button is visible.", "sunmi.scanner.inner", exampleParameters = """{}""", exampleOutput = """{"enabled":false}"""),
        MethodDescriptor("sunmi.scanner.inner.setVirtualFloatingScanButton", "Show or hide the virtual floating scan button.", "sunmi.scanner.inner", exampleParameters = """{"enabled":true}""", exampleOutput = """{}"""),
        MethodDescriptor("sunmi.scanner.inner.getPrefix", "Get the current scan result prefix.", "sunmi.scanner.inner", exampleParameters = """{}""", exampleOutput = """{"content":"PRE-"}"""),
        MethodDescriptor("sunmi.scanner.inner.setPrefix", "Set or clear the scan result prefix.", "sunmi.scanner.inner", exampleParameters = """{"content":"PRE-"}""", exampleOutput = """{}"""),
        MethodDescriptor("sunmi.scanner.inner.getSuffix", "Get the current scan result suffix.", "sunmi.scanner.inner", exampleParameters = """{}""", exampleOutput = """{"content":"-SUF"}"""),
        MethodDescriptor("sunmi.scanner.inner.setSuffix", "Set or clear the scan result suffix.", "sunmi.scanner.inner", exampleParameters = """{"content":"-SUF"}""", exampleOutput = """{}"""),
        MethodDescriptor("sunmi.scanner.inner.getPrefixCharactersRemoved", "Get number of prefix characters removed from scan result.", "sunmi.scanner.inner", exampleParameters = """{}""", exampleOutput = """{"length":0}"""),
        MethodDescriptor("sunmi.scanner.inner.setPrefixCharactersRemoved", "Set number of prefix characters to remove (0-20).", "sunmi.scanner.inner", exampleParameters = """{"length":2}""", exampleOutput = """{}"""),
        MethodDescriptor("sunmi.scanner.inner.getSuffixCharactersRemoved", "Get number of suffix characters removed from scan result.", "sunmi.scanner.inner", exampleParameters = """{}""", exampleOutput = """{"length":0}"""),
        MethodDescriptor("sunmi.scanner.inner.setSuffixCharactersRemoved", "Set number of suffix characters to remove (0-20).", "sunmi.scanner.inner", exampleParameters = """{"length":2}""", exampleOutput = """{}"""),
        MethodDescriptor("sunmi.scanner.inner.isRemoveGroupSeparator", "Check if group separator removal is enabled.", "sunmi.scanner.inner", exampleParameters = """{}""", exampleOutput = """{"enabled":false}"""),
        MethodDescriptor("sunmi.scanner.inner.setRemoveGroupSeparator", "Enable or disable group separator removal.", "sunmi.scanner.inner", exampleParameters = """{"enabled":true}""", exampleOutput = """{}"""),
        MethodDescriptor("sunmi.scanner.inner.isAdvancedFormatEnabled", "Check if advanced formatting is enabled.", "sunmi.scanner.inner", exampleParameters = """{}""", exampleOutput = """{"enabled":false}"""),
        MethodDescriptor("sunmi.scanner.inner.setAdvancedFormatEnabled", "Enable or disable advanced formatting.", "sunmi.scanner.inner", exampleParameters = """{"enabled":true}""", exampleOutput = """{}"""),
        MethodDescriptor("sunmi.scanner.inner.getAdvancedFormats", "Get all advanced format search/replace pairs.", "sunmi.scanner.inner", exampleParameters = """{}""", exampleOutput = """{"formats":{"search1":"replace1"}}"""),
        MethodDescriptor("sunmi.scanner.inner.setAdvancedFormats", "Replace all advanced format pairs.", "sunmi.scanner.inner", exampleParameters = """{"formats":{"old":"new"}}""", exampleOutput = """{}"""),
        MethodDescriptor("sunmi.scanner.inner.addAdvancedFormat", "Add an advanced format search/replace pair.", "sunmi.scanner.inner", exampleParameters = """{"search":"old","replacement":"new"}""", exampleOutput = """{}"""),
        MethodDescriptor("sunmi.scanner.inner.removeAdvancedFormat", "Remove an advanced format pair by search key.", "sunmi.scanner.inner", exampleParameters = """{"search":"old"}""", exampleOutput = """{}"""),
        MethodDescriptor("sunmi.scanner.inner.clearAdvancedFormats", "Remove all advanced format pairs.", "sunmi.scanner.inner", exampleParameters = """{}""", exampleOutput = """{}"""),
        MethodDescriptor("sunmi.scanner.inner.getBarcodesList", "Get all barcode symbologies with enabled/disabled status.", "sunmi.scanner.inner", exampleParameters = """{}""", exampleOutput = """{"barcodes":[{"name":"QR Code","enabled":true}]}"""),
        MethodDescriptor("sunmi.scanner.inner.getBarcode", "Get enabled/disabled status of a specific barcode type.", "sunmi.scanner.inner", exampleParameters = """{"name":"QR Code"}""", exampleOutput = """{"name":"QR Code","enabled":true}"""),
        MethodDescriptor("sunmi.scanner.inner.setBarcode", "Enable or disable a specific barcode type.", "sunmi.scanner.inner", exampleParameters = """{"name":"QR Code","enabled":true}""", exampleOutput = """{}"""),
        MethodDescriptor("sunmi.scanner.inner.getBarcodeConfig", "Get detailed configuration for a barcode symbology.", "sunmi.scanner.inner", exampleParameters = """{"name":"Code 128"}""", exampleOutput = """{"name":"Code 128","minLen":0,"maxLen":255}"""),
        MethodDescriptor("sunmi.scanner.inner.setBarcodeConfig", "Set configuration fields for a barcode symbology.", "sunmi.scanner.inner", exampleParameters = """{"name":"Code 128","minLen":4,"maxLen":40}""", exampleOutput = """{}"""),
    )
