...
/**
 * Unified control surface for a barcode-scanner backend behind the server-side `scanner` interface.
 *
 * Trigger a scan; decoded barcodes arrive asynchronously via {@link IScanner.onScan}. A concrete
 * client is bound to a single backend, so `onScan` delivers only that scanner's results (filtered
 * by event source).
 */
